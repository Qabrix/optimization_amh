...
class Population:
    ...
    def _random_word(self):
        random.sample(self.all_puzzle_keys, len(self.all_puzzle_keys))
    def _gen_generation(self):
        generation = []
        for _ in range(self.population_size):
            word = random.sample(self.all_puzzle_keys, len(self.all_puzzle_keys))
    ...
    def mutate(self, swap_probability=0.5, inverse_probability=0.01, random_probability=0.01):
        for inhabitant in self.generation:
