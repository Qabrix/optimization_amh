...
class Population:
    ...
    def _gen_generation(self):
        return [
            Inhabitant(random.shuffle(list(self.puzzles)))
        ...
        for inhabitant in self.generation:
            for i in range(len(inhabitant.gene)):
                if decision(probability):
                    inhabitant.gene[i], inhabitant.gene[random.randint(0,len(inhabitant))] = inhabitant.gene[random.randint(0,len(inhabitant))], inhabitant.gene[i]
