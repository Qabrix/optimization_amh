    ...
    def recombinate(self):
        ...
        for i in range(0, len(permutation)):
            ...
            if self._check_if_correct(new_word):
                ...
                new_generation.append(Inhabitant( selection[permutation[i%len(permutation)]]))

            new_word = selection[permutation[(i + 1)%len(permutation)]][:pivot] + selection[permutation[i%len(permutation)]][pivot:]
            if self._check_if_correct(new_word):
                new_generation.append(Inhabitant(new_word))
            else:
                new_generation.append(Inhabitant(selection[permutation[(i + 1)%len(permutation)]]))
