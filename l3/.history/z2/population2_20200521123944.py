...
class Population:
    ...
    def make_selection(self, percentage=0.5, elite_percentage=0.05):
        selection = []
    ...
    def recombinate(self):
        ...
        for i in range(1, len(permutation)):
            pivot = random.randint(0, min(len(selection[permutation[i % len(permutation)]]), len(selection[permutation[(i + 1) % len(permutation)]])) // 2)
    ...
    def mutate(self, min_swap_probability=0.1, max_swap_probability=0.2, inverse_probability=0.01, random_probability=0.01, shift_probability=0.01, insert_probability=0.01):
