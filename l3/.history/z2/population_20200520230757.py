...
class Population:
    ...
    def recombinate(self):
        ...
        permutation = np.random.permutation(len(selection)*2)
        new_generation = []
        for i in range(0, len(permutation)-1):
            ...
            new_generation.append(
                Inhabitant(
                    selection[permutation[i]%len(selection)][:pivot1]
                    + selection[permutation[i + 1]% len(selection)][pivot1:pivot2]
                    + selection[permutation[i]% len(selection)][pivot2:]
                )
            )
            new_generation.append(
                Inhabitant(
                    selection[permutation[i + 1]% len(selection)][:pivot1]
                    + selection[permutation[i]% len(selection)][pivot1:pivot2]
                    + selection[permutation[i + 1]% len(selection)][pivot2:]
                )
            )
