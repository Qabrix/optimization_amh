...
class Population:
    ...
    def recombinate(self, elite_percentage=0.6):
        ...
        for i in range(1, len(permutation)):
            ...
            if len(selection[permutation[(i+1) % len(permutation)]]) > 1:
                pivot2 = random.randint(len(selection[permutation[(i+1) % len(permutation)]])//2, len(selection[permutation[(i+1) % len(permutation)]])-1)
            else:
                pivot2 = 1
            new_word = selection[permutation[i % len(permutation)]][:pivot2]
            if self._check_if_correct(new_word):
