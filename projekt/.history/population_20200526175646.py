...
class Population:
    ...
    def _semi_backtrack_removal(self, path):
        ...
        for remove_index1 in range(len(path)):
            if path[i] == opposite:
                remove_index2 = i-1
                break
        
        return path[:remove_index1] + path[remove_index1+1:] +path[:remove_index1] + path[remove_index1+1:] 
    ...
    def mutate(
        ...
        for inhabitant in self.generation[1:]:
            ...
            if decision(semi_backtrack_removal_probability) and len(inhabitant) > 2:
                inhabitant.gene = self._semi_backtrack_removal()
                continue
